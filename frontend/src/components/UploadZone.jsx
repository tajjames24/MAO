  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[70vh] gap-10 animate-fade-up">
      ...
      <div className="text-center space-y-4 max-w-2xl">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-[#CC5500]/10 text-[#CC5500] rounded-full text-xs font-semibold uppercase tracking-widest mb-2">
          <span className="w-1.5 h-1.5 bg-[#CC5500] rounded-full" />
        ...
        <h1 className="text-5xl md:text-7xl font-extrabold tracking-tighter leading-none text-gray-900 font-manrope">
          Find Your<br />
          <span className="text-[#CC5500]">Musical Key</span>
        </h1>
      ...
      <div
        ...
        className={`relative w-full max-w-2xl rounded-3xl border-2 border-dashed transition-colors duration-300 cursor-pointer group overflow-hidden
          ${isDragActive
            ? "border-[#CC5500] bg-[#CC5500]/5 shadow-[0_0_0_6px_rgba(255,90,31,0.08)]"
            : isDragReject
            ? "border-red-400 bg-red-50"
            : "border-gray-200 bg-gray-50 hover:border-[#CC5500]/50 hover:bg-[#CC5500]/[0.02]"
          }`}
        style={{ minHeight: 300 }}
      >
        ...
        <div className="absolute inset-0 flex items-end justify-center gap-0.5 px-12 pb-6 opacity-10 pointer-events-none">
          {Array.from({ length: 48 }).map((_, i) => (
            <div
              key={i}
              className="w-1 bg-[#CC5500] rounded-full"
        ...
        <div className="relative flex flex-col items-center justify-center py-16 px-8 gap-5">
          <div className={`w-20 h-20 rounded-2xl flex items-center justify-center transition-colors duration-300
            ${isDragActive ? "bg-[#CC5500]" : "bg-white border-2 border-gray-200 group-hover:border-[#CC5500]/40"}`}>
            {isDragActive ? (
              <Music className="w-9 h-9 text-white" strokeWidth={1.5} />
            ) : (
              <Upload className="w-9 h-9 text-gray-400 group-hover:text-[#CC5500] transition-colors" strokeWidth={1.5} />
            )}
          </div>
          ...
          <div className="text-center space-y-1.5">
            ...
            <p className="text-sm text-gray-500">
              or <span className="text-[#CC5500] font-medium">click to browse</span>
            </p>
          </div>
          ...
          <div className="flex gap-2 mt-2">
            {["MP3", "WAV", "M4A"].map((fmt) => (
              ...
              >
                <FileAudio className="w-3 h-3 text-[#CC5500]" />
                {fmt}
              </span>
            ))}
          </div>
      ...
      <div className="grid grid-cols-3 gap-6 max-w-lg w-full text-center animate-fade-up stagger-3">
        {[
          ...
          <div key={step} className="space-y-1">
            <div className="text-xs font-mono font-bold text-[#CC5500] uppercase tracking-widest">{step}</div>
