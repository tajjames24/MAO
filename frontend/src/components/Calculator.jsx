...
import {
  ...
  BookOpen, Copy, FileText, TrendingUp, X, Info
} from 'lucide-react';
...
export default function Calculator() {
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(20);
    doc.text('WHOLESALE OFFER CALCULATOR', 14, 19);
    ...
    doc.text('Generated by Wholesale Offer Calculator — Quick Deal Analyzer for Real Estate Wholesalers', 14, 285);
  ...
  const copyToClipboard = async () => {
    const text = `WHOLESALE DEAL SUMMARY
======================
      ...
      <header className="sticky top-0 z-50 bg-white border-b-2 border-[#1A1A1A]">
        <div className="max-w-7xl mx-auto px-4 md:px-8 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 bg-[#FF7A00] border-2 border-[#1A1A1A] flex items-center justify-center shrink-0">
              <TrendingUp className="w-3.5 h-3.5 text-white" strokeWidth={2.5} />
            </div>
            <div>
              <span className="font-black text-[#1A1A1A] text-base leading-none" style={{ fontFamily: 'Chivo, sans-serif' }}>
                WholesaleCalc
              </span>
            </div>
          </div>
